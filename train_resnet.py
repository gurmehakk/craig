    ...
    for run in range(runs):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if subset_size >= 1 or epoch < args.start_subset:
                ...
                if greedy == 0:
                    ...
                    if args.cluster_features:
                        ...
                        preds, labels = predictions(indexed_loader, model,device)
            ...
            data_time[run, epoch], train_time[run, epoch] = train(
                train_loader, model, train_criterion, optimizer, epoch, weight,device)

            lr_scheduler_f.step()

            # evaluate on validation set
            prec1, loss = validate(val_loader, model, val_criterion,device)
            ...
            ta, tl = validate(train_val_loader, model, val_criterion,device)
...
def train(train_loader, model, criterion, optimizer, epoch, weight=None,device=None):
    """
        Run one train epoch
    """
    if weight is None:
...
def validate(val_loader, model, criterion,device=None):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            # measure accuracy and record loss
            prec1 = accuracy(output.data, target.to(device))[0]
            losses.update(loss.item(), input.size(0))
            top1.update(prec1.item(), input.size(0))
...
def predictions(loader, model,device):
    """
    Get predictions
    """
    batch_time = AverageMeter()
